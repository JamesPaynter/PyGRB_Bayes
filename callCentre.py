...
def load_8099(sampler = 'dynesty', nSamples = 100):
    object = BilbyObject(8099, times = (2, 15),
                datatype = 'discsc', nSamples = 300, sampler = sampler,
                priors_pulse_start = 0, priors_pulse_end = 15)
    return object


def compare_FRED_FREDx(function, sampler = 'Nestle', **kwargs):
    GRB = function(sampler, **kwargs)
    evidences_1_FRED,  errors_1_FRED  = GRB.one_FRED( channels = [0,1,2,3], test = False)
    evidences_1_FREDx, errors_1_FREDx = GRB.one_FREDx(channels = [0,1,2,3], test = False)
    for i in range(4):
        print('---------------')
