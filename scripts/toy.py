...
grb2 = BATSESignal(3770, 'tte', times = (-0.132, 1.45))
# plt.plot(grb2.bin_left, grb2.counts)
# plt.show()
...
y+= 0.01
y_counts = np.random.poisson(y)
# plt.plot(time, y)
# plt.plot(time, y_counts)
# plt.plot(grb.bin_left, grb.counts[:,2])
# plt.show()
print(grb.bin_left)
idx = np.digitize(grb.bin_left, grb2.bin_left)

counts = np.zeros((idx[-1]+1, 4))
count2 = np.zeros((idx[-1]+1))

for i in range(len(grb.bin_left)):
    # print(grb.counts[i,2], counts[idx[i]])
    counts[idx[i],:] += grb.counts[i,:]
    count2[idx[i]]   += y_counts[i]
    # break
print(counts)

for i in range(4):
    plt.plot(grb2.bin_left, counts[:-1,i], drawstyle = 'steps-mid',
                color = grb.colours[i])
